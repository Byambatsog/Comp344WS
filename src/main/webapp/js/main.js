...
$('#btnCustomerSignup').live('click', function() {
	...
	if(email == '') {
		window.alert("You must inter email");
		return;
	}
	var password = $('#signupPassword').val();
	if(password == '') {
		window.alert("You must inter password");
		return;
	}

	var firstName = $('#firstName').val();
	if(firstName == '') {
		window.alert("You must inter first name");
		return;
	}

	var lastName = $('#lastName').val();
	if(lastName == '') {
		window.alert("You must inter last name");
		return;
	}
...
function renderOrder(order){
	...
	var content = '<div class="detailItem"><table cellpadding="5" cellspacing="0">';
	content += '<thead><td>#</td><td>Name</td><td>Price</td><td>Q</td><td>X</td></thead>';

	$.each(list, function(index, product) {
		content += '<tr>';
		content += '<td>' + (index + 1) + '.</td><td>' + product.name + '</td><td>$' + product.unitPrice + '</td><td>' + product.quantity + '</td><td>X</td>';
		content += '</tr>';
		totalPrice += product.unitPrice * product.quantity;
	});
	content += '</table></div>';
	...
	content += '<div class="orderItem"><label>Status</label><span>' + order.lastStatus + '</span></div>';

	content += '<div class="orderItem"><button id="cancelOrder" onclick="cancelOrder();">Cancel an order</button></div>';
